...
def process_input():
    ...
    if keys[pygame.K_t] and not previous_keys[pygame.K_t]:
        ...
        if training:
            ...
            position = rsa.get_best_position()
...
# Defining rsa hyperparameters
hyperparams = Params()

hyperparams.objective="max"
hyperparams.name="Algorithm RSA"
hyperparams.range_min=np.array([0.0, 10.0, 0.0, 0.0])
hyperparams.range_max=np.array([0.9, 200.0, 1300.0, 30.0])
hyperparams.n=40
hyperparams.d=4
hyperparams.alpha=0.1
hyperparams.beta=0.005
hyperparams.max_iter=100
#rsa = RSA(hyperparams)
rsa=RSA(obj="max", lb=[0.0, 10.0, 0.0, 0.0], ub=[0.9, 200.0, 1300.0, 30.0], dim=4, n=40, max_iter=1000)
...
pygame.display.set_caption("Experiment 3 - Lower - Line Follower Optimization")
...
while run:
    ...
    for i in range(num_steps):
        ...
        if episode_time >= MAX_EPISODE_TIME:
            ...
            f.write('iter: ' + str(training_iteration) +
                  ', ' + 'params: ' + format_position(position) + ', quality: ' + str(quality))
            if training:
                ...
                rsa.notify_evaluation(quality)
                position = rsa.get_position_to_evaluate()
                pd.DataFrame([training_iteration,middle_time-comeco,position]).to_pickle("position_history.pickle")
                pd.DataFrame([training_iteration,middle_time-comeco,quality]).to_pickle("quality_history.pickle")
            else:
